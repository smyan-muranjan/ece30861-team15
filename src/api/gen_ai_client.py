...
class GenAIClient:
    ...
    def preprocess_readme_for_analysis(
        self, readme_text: str, max_chars: int = 3000
    ) -> str:
        """Extract relevant sections from README
        and limit size for reliable processing."""
        if not readme_text:
            return ""
        ...
        perf_keywords = ['performance', 'benchmark', 'evaluation', 'results',
                         'metrics', 'accuracy', 'score', 'f1', 'precision',
                         'recall', 'bleu', 'rouge', 'evaluation', 'test',
                         'dataset', 'model', 'experiment']

        # Keywords for documentation quality
        doc_keywords = ['installation', 'install', 'setup', 'usage', 'example',
                        'quickstart', 'getting started', 'tutorial', 'guide',
        ...
        for i, line in enumerate(lines):
            ...
            if is_header and any(
                keyword in line_lower for keyword in perf_keywords
            ):
            ...
            elif is_header and any(
                keyword in line_lower for keyword in doc_keywords
            ):
                if current_section and capture_mode:
                    sections.append(('\n'.join(current_section), capture_mode))
                current_section = [line]
            ...
            # Stop capturing when hitting unrelated sections
                ...
                keywrd in line_lower for keywrd in perf_keywords + doc_keywords
            ):
                if current_section:
                    sections.append(('\n'.join(current_section), capture_mode))
                current_section = []
        ...
        if not sections:
            ...
            if re.search(
                r'\d+\.?\d*\s*%|accuracy|f1|score|benchmark',
                middle_part.lower()
            ):
                return (
                    first_part.strip() +
        ...
        performance_sections = [
            s[0] for s in sections if s[1] == 'performance'
        ]
    ...
    def _fallback_performance_parsing(self, readme_text: str) -> dict:
        ...
        benchmark_keywords = [
            'glue', 'superglue', 'imagenet', 'coco', 'squad', 'squad2.0',
            'squad 2.0',
            'librispeech', 'commonvoice', 'wmt', 'bleu', 'rouge', 'meteor',
            'benchmark', 'evaluation', 'dataset', 'test set', 'validation set',
            'leaderboard', 'competition', 'challenge', 'sota',
            'state-of-the-art'
        ]

        mentions_benchmarks = any(
    ...
    def _fallback_clarity_assessment(self, readme_text: str) -> float:
        ...
        clarity_indicators = [
            ...
            (['install', 'installation', 'setup',
              'getting started', 'quickstart'], 0.2),
            # Usage information
    ...
    def _extract_json_from_response(self, response: str) -> dict | None:
        ...
        json_matches = re.findall(
            r'\{[^{}]*"has_metrics"[^{}]*"mentions_benchmarks"[^{}]*\}',
            response
        )
            ...
            try:
                ...
                if "has_metrics" in parsed and "mentions_benchmarks" in parsed:
                    return {
                        ...
                        "mentions_benchmarks": int(
                            parsed["mentions_benchmarks"]
                        )
                    }
            ...
            try:
                ...
                if isinstance(parsed, dict):
                    ...
                    for key, value in parsed.items():
                        key_low = key.lower()
                        if 'metric' in key_low or 'performance' in key_low:
                            has_metrics = int(bool(value))
                        elif 'benchmark' in key_low or 'evaluation' in key_low:
    ...
    def _extract_float_from_response(self, response: str) -> float | None:
        ...
        try:
            ...
            for prefix in ["score:", "clarity:", "rating:",
                           "the score is", "result:"]:
                if cleaned.lower().startswith(prefix):
                    cleaned = cleaned[len(prefix):].strip()

            if all(c.isdigit() or c == '.' for c in cleaned.strip()):
                score = float(cleaned)
    ...
    async def get_performance_claims(self, readme_text: str) -> dict:
        ...
        processed_readme = self.preprocess_readme_for_analysis(
            readme_text, max_chars=3000
        )
        ...
        user_prompt = user_prompt_template.format(
            processed_readme=processed_readme
        )
        ...
        try:
            ...
            logging.warning(
                f"JSON parsing failed, using fallback for response: "
                f"{response[:200]}..."
            )
    ...
    async def get_readme_clarity(self, readme_text: str) -> float:
        ...
        # Preprocess README - limit to first 2000
        # characters for clarity assessment
        ...
        try:
            ...
            logging.warning(
                "Float parsing failed, using fallback for response: "
                f"{response[:100]}..."
            )
