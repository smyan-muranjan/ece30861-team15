...
class GitClient:
    ...
    def analyze_code_quality(self, repo_path: str) -> CodeQualityStats:
        ...
        try:
            ...
            has_tests = any(
                any(repo_path_obj.rglob(f"{pattern}*")) for pattern in test_patterns
            )
            ...
            try:
                ...
                if python_files:
                    ...
                    result = subprocess.run(
                        ['flake8', '--count', '--quiet'] + [str(f) for f in python_files],
                        capture_output=True,
                        text=True,
                        cwd=repo_path
                    )
    ...
    def analyze_ramp_up_time(self, repo_path: str) -> RampUpStats:
        ...
        try:
            ...
            # Analyze README quality (simplified - in real implementation, use LLM)
            ...
            if readme_files:
                ...
                try:
                    ...
                    readme_quality = sum(quality_indicators) / len(quality_indicators)
                except Exception as e:
