...
class LocalMetricsCalculator:
    ...
    def calculate_code_quality(self, repo_path: str) -> tuple[float, int]:
        ...
        try:
            ...
            logging.info(
                f"Code quality calculated: {quality_stats.code_quality_score}")
    ...
    def calculate_ramp_up_time(self, repo_path: str) -> tuple[float, int]:
        ...
        try:
            ...
            logging.info(
                f"Ramp-up time calculated: {ramp_up_stats.ramp_up_score}")
    ...
    def calculate_size_score(
            self, repo_path: str) -> tuple[Dict[str, float], int]:
        """
    ...
    def analyze_repository(self, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            bus_factor, bus_factor_latency = \
                self.calculate_bus_factor(repo_path)
            code_quality, code_quality_latency = \
                self.calculate_code_quality(repo_path)
            ramp_up_time, ramp_up_time_latency = \
                self.calculate_ramp_up_time(repo_path)
            size_scores, size_score_latency = \
                self.calculate_size_score(repo_path)
